...
integer_column_index <- rep(NA, dim(mtcars)[2])

for (i in seq_along(integer_column_index)) {
  integer_column_index[i] <- all(mtcars[[i]] == floor(mtcars[[i]]))
}

mtcars[, integer_column_index]
...
mtcars %>% select_if(~ all(.x == floor(.x)))

#renaming...
starwars %>%
  select(height, weight = mass)
...
starwars %>%
  ...
  summarise(
    row_counts = n(),
    mass_mean = mean(mass, na.rm = TRUE),
    height_mean = mean(mass, na.rm = TRUE),
...
# let's calculate gdp_per_capita in most recent year and calculate top 3 
